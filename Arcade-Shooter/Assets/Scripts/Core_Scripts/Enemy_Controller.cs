...
public class Enemy_Controller : MonoBehaviour
{
...

    {
        ...
        while (waveType < 1)
        {
            ...
            {
                for (int b = 0; b < _Waves[j].Objects.Length; b++)
                {
                    _Waves[b].EnemiesList = GameManager.ObjectPooler(_Waves[j].Objects[b], _Waves[j].MaxEnemies[b]);

                    for ( i = 0; i < _Waves[j].MaxEnemies.Length; i++)
                    {
