...
public class Jun_MultiLanguageEditor : Jun_PopupWindow
{
    public static void Open(Rect rect, SerializedProperty property, string name)
    {
        ...
        windows.position = new Rect(rect.x, rect.y, 420, 50);//rect.width
    ...
    public override void OnGUI()
    {
        ...
        if (languages.arraySize > 0)
        {
            for (int i = 0; i < languages.arraySize; i++)
            {
                ...
                EditorGUILayout.PropertyField(systemLanguage, new GUIContent(""), GUILayout.Width(60));
                languageText.stringValue = EditorGUILayout.TextArea(languageText.stringValue, GUILayout.Width(110));

                EditorGUILayout.LabelField(Farsi.faConvert(languageText.stringValue), GUILayout.Width(110));
